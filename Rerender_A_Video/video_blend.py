...
def create_sequence(base_dir, beg, end, interval, key_frames_dir, input_video_dir, output_frames_subdir):
    if input_video_dir.endswith('.mp4'):
        reader = imageio.get_reader(input_video_dir)
        for i_frame, im in enumerate(reader):
            Image.fromarray(im).resize((512, 512))\
                .save(f'./tmp_video_frames/{i_frame:05}.png')

        input_video_dir = './tmp_video_frames'

    sequence = VideoSequence(base_dir=base_dir,
                             beg_frame=beg,
                             end_frame=end,
                             interval=interval,
                             input_video_dir=input_video_dir,
                             key_frames_dir=key_frames_dir,
                             blending_out_subdir=output_frames_subdir,
                             tmp_subdir='tmp',
                             input_format='%05d.png',
                             key_format='%05d.png',
...
def process_seq(video_sequence: VideoSequence,
    ...
    for i in range(interval - 1):
        ...
        # cv2.imwrite(f'mask/mask_{c_id:05d}.jpg', out_mask * 255)
...
def main(args):
    ...
    video_sequence = create_sequence(f'{args.name}', args.beg, args.end,
                                     args.itv, args.key_frames_dir, args.input_video_dir, args.output_frames_subdir)
    if not args.ne:
    ...
    if args.output:
        frame_to_video(video_path=args.output,
                       frame_dir=video_sequence.blending_dir,
                       fps=args.fps,
                       log=False)
    if not args.tmp:
...
if __name__ == '__main__':
    ...
    parser.add_argument('name', type=str, help='Path to working dir')
    ...
    parser.add_argument('--key_frames_dir',
                        type=str,
                        required=True,
    ...
    parser.add_argument('--input_video_dir',
                        type=str,
                        required=True,
                        help='Dir of input video frames')
    parser.add_argument('--output_frames_subdir',
                        type=str,
                        required=True,
                        help='Subdir of output video frames')
