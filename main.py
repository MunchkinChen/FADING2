...
parser.add_argument('--prompt_src', required=True, type=str)
parser.add_argument('--prompt_tgt', required=True, type=str)
parser.add_argument('--i_mask_word', required=True, type=str)
...
i_mask_word = [args.i_mask_word] if args.i_mask_word else []
...
for i, t in enumerate(tqdm(forward_scheduler.timesteps.tolist(),
    ...
    if attnStore.self_attention_blend and i > 40:
        ...
        attn_mask = attnStore.cross_attn_mask.gt(0).to(dtype=weight_dtype).unsqueeze(0).unsqueeze(0)
...
save_path = f"./output/{video_name}-{prompt_tgt}"
...
for i in range(edited_video.shape[2]):
    ...
    Image.fromarray(frame).save(os.path.join(save_path, f'{i_frame:05}.png'))
