...
# class UNet2DConditionModelInflated(ModelMixin, ConfigMixin):
class UNet2DConditionModelInflated(UNet2DConditionModel):
    r"""
    UNet2DConditionModel is a conditional 2D UNet model that takes in a noisy sample, conditional state, and a timestep
    and returns sample shaped output.

    This model inherits from [`ModelMixin`]. Check the superclass documentation for the generic methods the library
    implements for all the models_ (such as downloading or saving, etc.)
    ...
    Parameters:
        ...
        resnet_time_scale_shift (`str`, *optional*, defaults to `"default"`): Time scale shift config
            for resnet blocks, see [`~models_.resnet.ResnetBlock2D`]. Choose from `default` or `scale_shift`.
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`models_.unet_2d_condition.UNet2DConditionOutput`] instead of a plain tuple.

        Returns:
            [`~models_.unet_2d_condition.UNet2DConditionOutput`] or `tuple`:
            [`~models_.unet_2d_condition.UNet2DConditionOutput`] if `return_dict` is True, otherwise a `tuple`. When
    ...
    #     from diffusers.models_.modeling_utils import load_state_dict
    #
