...
#     VIDEO_NAME = param[0]
#     PROMPT_TGT = param[2]
#     cmd = f"rsync -avz --progress \"xchen-21@gpu-gw:'/home/ids/xchen-21/FADING2/output_blender/{VIDEO_NAME}-{PROMPT_TGT}.mp4'\" \"../output_blender/{VIDEO_NAME}-{PROMPT_TGT}.mp4\""
#     print(cmd)
...
for param in params:
    # param_ = [f"'{p}'" for p in param]
    VIDEO_NAME = param[0]
    cmd = f"rsync -avz --progress \"xchen-21@gpu-gw:'/home/ids/xchen-21/FADING2/output_blender/{VIDEO_NAME}-input.mp4'\" \"../output_blender/{VIDEO_NAME}-input.mp4\""
    print(cmd)
    process = subprocess.run(cmd, shell=True, capture_output=True)
    print(process.stdout)
